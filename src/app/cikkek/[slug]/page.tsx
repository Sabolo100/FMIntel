        ...
        <article>
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            ...
            <span className="text-xs text-slate-400">
              {typeLabel[article.type] ?? article.type} · {styleLabel[article.style] ?? article.style}
            </span>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {article.recoveryTime && (
              <span className={`inline-flex items-center gap-1 text-xs font-medium px-2.5 py-0.5 rounded-full ${article.recoveryTime.className}`}
            ...
            {article.intensityZone && (
              <span className={`inline-flex items-center gap-1 text-xs font-medium px-2.5 py-0.5 rounded-full ${article.intensityZone.className}`}
                title={article.intensityZone.description}>
                <svg className="w-3 h-3 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
                </svg>
                {article.intensityZone.zone} – {article.intensityZone.description}
              </span>
            )}
            {article.ageBadge && (
              <span className={`inline-flex items-center gap-1 text-xs font-medium px-2.5 py-0.5 rounded-full ${article.ageBadge.className}`}
                title="Célkorosztály">
                <svg className="w-3 h-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {article.gearLevel && (
              <span className={`inline-flex items-center gap-1 text-xs font-medium px-2.5 py-0.5 rounded-full ${gearLevelConfig[article.gearLevel].className}`}
            ...
            {/* Edzésterv extra meta */}
            {article.weeksDuration && (
              <span className="inline-block text-xs font-medium px-2.5 py-0.5 rounded-full bg-brand-50 text-brand-700 border border-brand-200">
                {article.weeksDuration} hét
              </span>
            )}
            {article.sessionsPerWeek && (
              <span className="inline-block text-xs font-medium px-2.5 py-0.5 rounded-full bg-brand-50 text-brand-700 border border-brand-200">
                Heti {article.sessionsPerWeek}x
              </span>
            )}
            {article.difficulty && (
              <span className="inline-block text-xs font-medium px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-700 border border-slate-200 capitalize">
                {article.difficulty}
              </span>
            )}
          </div>
